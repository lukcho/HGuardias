...
import hguardias.model.access.Menu;
import hguardias.model.manager.ManagerAcceso;
	...
	public String logIn(){
		try {
			if(getUsuario()==null || getUsuario().isEmpty() || getPass()==null || getPass().isEmpty()){
				...
				setMenu(mngAcc.loginWS(getUsuario(), getPass(), "HGUA"));
				setPass(null);
	...
	public String validarSesion(String vista){
	     ...
	     if (user==null || user.getUsuario() == null) {
        	...
        	if(ma.poseePermiso(vista, user.getMenu()))
        		...
        		try {
       				FacesContext.getCurrentInstance().getExternalContext().redirect("/hguardias/views/index.xhtml");
	            } catch (IOException ex) {
	            	Mensaje.crearMensajeERROR(ex.getMessage());
	            }
	            return null;
