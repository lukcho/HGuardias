...
public class lugarBean implements Serializable {
		...
		if (lug_id != null) {
			if (lug_idTurno != -1) {
				if (!numeroGuardias.equals("") && !numeroGuardias.equals("0")) {
					...
					Mensaje.crearMensajeWARN("Debe agregar un valor a n�mero de guardias");
				}
			} else {
	...
	public String cambiarEstadoLugar() {
		try {
			...
			context.addMessage(null, new FacesMessage("INFORMACI�N",
	...
	public void guardarPrioridad() throws Exception {
		...
		Mensaje.crearMensajeINFO("Actualizaci�n de lugares");
		listaLugares = managergest.findAllLugares();
	...
	/**
	 * M�todo para activar y desactivar estado del item
	 * 
	 * @param pro_id
	 * @throws Exception
	 */
